...
function HueLight (accessory, id, obj, type = 'light') {
  ...
  this.setConfig()
  this.log.debug("Erik: HueLight config: " + JSON.stringify(this.config))
  ...
  if (this.config.windowCovering) {
    ...
    if (this.config.outlet) {
      ...
      this.log.debug("Creating new Service.Lightbulb for light with name %s and subtype %s", this.name, this.subtype)
