...
function HueLight (accessory, id, obj, type = 'light') {
  ...
  if (this.config.windowCovering) {
    ...
    if (this.config.outlet) {
      ...
      this.log.debug("Creating new Service.Lightbulb for light with name %s, type %s and subtype %s", this.name, this.type, this.subtype)
  ...
  if (this.type === 'light') {
    ...
    if (this.bridge.config.nativeHomeKitLights) {
      this.log.debug("Configuration nativeHomeKitLights for light with name %s", this.name)
