...
HueBridge.prototype.accessories = async function () {
  ...
  try {
    ...
    // A JSON object containing all resources (accessories, services)
    // this bridge offers.
    const state = await this.getFullState()
    // From the JSON state, extract the accessories etc. and expost
    // them to homebridge
  ...
  this.log.info('%s: %d accessories: %s', this.name, this.accessoryList.length, this.accessoryList.map(a => a.name).join(", "))
  return this.accessoryList
}
...
HueBridge.prototype.getFullState = async function () {
  let state = await this.request('get', '/')
  ...
  // Main fix for Erik: deCONZ takes some time to disclose the lights
  // after startup. This leads to the disappearance of the lights from
  // HomeKit since it looks like all lights had been gone and are thus
  // removed, losing all information given to them (room location,
  // rules, scenes).
  this.log.info("Num lights found: %s", JSON.stringify(Object.keys(state.lights)))
  this.log.info("Lights: %s", state.lights)
  while(Object.keys(state.lights).length == 0) {
    this.log.info("No lights have been disclosed from deCONZ. This version of Homebridge Hue requires at least one light to be available. Waiting for 15 seconds and trying again.")
    await homebridgeLib.timeout(15000)
    state = await this.request('get', '/')
  }
...
HueBridge.prototype.exposeResources = async function (obj) {
  ...
  this.wallswitch = {}
  this.obj = obj.config
  // obj is the parsed JSON information from the bridge dislosing
  // the resources offered by this homebridge
  for (const key in obj.resourcelinks) {
    // This loop only applies if the link name equals "homebridge-hue"!
    const link = obj.resourcelinks[key]
    if (link.name === 'homebridge-hue' && link.links && link.description) {
      this.log.warn("Erik: I think I never come here")
      const list = link.description.toLowerCase()
      // The cases list the keys we can handle. All other keys
      // trigger the continue statement, proceding to the next resource
      switch (list) {
        case 'blacklist':
        case 'lightlist':
        case 'multiclip':
      ...
      for (const resource of link.links) {
        ...
        // Check if the given state was a bad response
        // due to a not yet completed startup of the bridge
...
HueBridge.prototype.exposeSensor = function (id, obj) {
  ...
  if (obj.type[0] === 'Z') {
    ...
    if (a != null) {
      ...
      if (obj.type === 'ZHASwitch') {
        // Re-expose button tile in Home on iOS 14.
        if (obj.manufacturername === 'Samjin' && obj.modelid === 'button') {
          serialNumber += '-S'
        }
      }
    }
...
HueBridge.prototype.exposeLights = function (lights) {
  for (const id in lights) {
    ...
    this.log.debug("Got light with ID %s and name %s", id, light.name)
...
HueBridge.prototype.exposeLight = function (id, obj) {
  ...
  if (accessory == null) {
    this.log.debug("Light with ID %s and uniqueid %s is not yet known and added to the accessoryMap", id, uniqueid)
    accessory = new HueAccessory(this, serialNumber)
    this.accessoryMap[serialNumber] = accessory
  } else {
    this.log.debug("Light with ID %s and uniqueid %s was retrieved from the accessoryMap", id, uniqueid)
  }
