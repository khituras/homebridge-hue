...
HueBridge.prototype.exposeLight = function (id, obj) {
  ...
  const uniqueid = obj.uniqueid == null ? '' : obj.uniqueid
  this.log.debug("Light with ID %s has uniqueid: %s", id, uniqueid)
  const a = uniqueid.match(/(..:..:..:..:..:..:..:..)-(..)(:?-....)?/)
  ...
  if (accessory == null) {
    this.log.debug("Light with ID %s and uniqueid is not yet known and added to the accessoryMap", id, uniqueid)
    accessory = new HueAccessory(this, serialNumber)
    this.accessoryMap[serialNumber] = accessory
  } else {
    this.log.debug("Light with ID %s and uniqueid was retrieved from the accessoryMap", id, uniqueid)
  }
  this.log.debug("Light with serialNumber %s is added as light resource with ID %s", id, serialNumber)
